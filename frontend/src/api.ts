...
export type ApiFailureKind = "timeout" | "network" | "server" | "cancelled";
...
async function post<T>(path: string, body: unknown, signal?: AbortSignal): Promise<ApiResult<T>> {
...
export function sparringStep(turn: SparringTurn, signal?: AbortSignal): Promise<ApiResult<SparringResponse>> {
  return post<SparringResponse>("/api/sparring/step", turn, signal);
}

export function hearingStep(turn: HearingTurn, signal?: AbortSignal): Promise<ApiResult<HearingResponse>> {
  return post<HearingResponse>("/api/hearing/step", turn, signal);
}
