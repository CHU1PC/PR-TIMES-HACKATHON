    ...
    <section className="page home-page">
              ...
              {weeks.map((week, weekIndex) => (
                <tr key={`week-${String(weekIndex)}`}>
                  {week.map((day, weekdayIndex) => {
                    ...
                    const label = `${formatDate(day.key)}${event ? `、${event.title}` : ""}を選択`;
                    return (
                      <td key={day.key} className={!day.isCurrentMonth ? "is-outside" : undefined}>
                        <Link
                          to={entryPath(day.key, event?.title)}
                          className={`calendar-day${day.isToday ? " is-today" : ""}${event ? " has-event" : ""}`}
                          aria-label={label}
      ...
      <section id="ranking" className="ranking-card" aria-labelledby="ranking-title">
        ...
        <ol className="ranking-list">
          {monthlyRankedEvents.map((event, index) => (
            <li key={event.id} className="ranking-item">
              ...
              <Link to={entryPath(event.date, event.title)} className="ranking-item__action">
                この予定で作る
      ...
      <aside className="home-tip">
        ...
        <div className="home-tip__copy">
          <h2>PRを届きやすくするコツ</h2>
          <p>イベントの詳細情報を充実させることで、より具体的なPR提案を作れます。</p>
        </div>
        <Link to="/entry" className="home-tip__badge home-tip__badge--link">
